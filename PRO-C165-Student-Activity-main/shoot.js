AFRAME.registerComponent("bullets", {
  ...
  shootBullet: function () {
    window.addEventListener("keydown", (e) => {
      if (e.key === "z") {
        ...
        bullet.addEventListener("collide", this.bulletCollision);
 ...
 removeBullet: function (e) {
  ...
  bulletCollision:function(e){
    var element1 = document.querySelector("#countTank")
            var tankLife = parseInt(element1.getAttribute("text").value)
            var elementHit = e.detail.body.el;
            //collide event on enemy bullets
                if (e.detail.body.el.id === "enemy1"|| e.detail.body.id === "enemy2") {

                    //Add the conditions here
                    if(tankLife > 0){
                        tankLife -= 1
                        element1.setAttribute("text",{value:tankLife})
                    }
                    if(tankLife <= 0){
